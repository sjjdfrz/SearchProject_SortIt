...
class Problem:
        ...
        for i in range(len(state.pipes)):
            for j in range(len(state.pipes)):
                ...
                if not state.pipes[j].is_full() and not state.pipes[i].is_empty():
                    s = State(copy.deepcopy(state.pipes), state, self.get_cost_from_change(state, i, j), (i, j))
    ...
    def get_cost_from_change(self, state: State, pipe_src_ind: int, pipe_dest_ind: int) -> int:
